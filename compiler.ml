...
let file_to_string input_file =
  ...
  let rec run () =
    try 
      let ch = input_char in_channel in
      ch :: (run ())
    with End_of_file ->
      (close_in in_channel; [])
  in
...
module Code_Generation (* : CODE_GENERATION *) = struct
  ...
  let asm_comment_of_sexpr sexpr =
    ...
    let str =
      ...
      then str
      else (String.sub str 0 comment_length) ^ "..."
    in
    "; " ^ str;;
  ...
  let asm_of_representation sexpr =
    ...
    let run = function
      ...
      | [RTTI "T_interned_symbol"; ConstPtr addr] ->
          Printf.sprintf "\tdb T_interned_symbol\t%s + %d" str label_start_of_constants_table addr
  ...
  let collect_free_vars =
    ...
    and runs exprs' =
      List.fold_left
        ...
        exprs'
    in
    fun exprs' -> remove_duplicates (runs exprs');;
    ...
    String.concat "\n"
      (List.map
         (fun (scheme_name, asm_code_ptr) ->
           let free_var_label =
             search_free_var_table scheme_name free_var_table
           in
    ...
    let tmp = 
      List.map
        (fun (scm_var, asm_label) ->
          let addr =
            search_constant_address (ScmString scm_var) consts_table
          in
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmVarGet' (Var' (v, Free)) ->
          let label = search_free_var_table v free_var_table in
          (Printf.sprintf
    ...
    and runs params env exprs' =
      List.map (fun expr' -> run params env expr') exprs'
    in
