...
module Tag_Parser : TAG_PARSER = struct
  ...
  let is_list_of_unique_names =
    ...
    (*I rearranged the existing code to be more readable with more comments *)
    let rec tag_parse sexpr =
      match sexpr with
      (* ----------------------------------------------------------------------
         1) Constants: void, booleans, chars, strings, numbers
         ---------------------------------------------------------------------- *)
      | ScmVoid
      | ScmBoolean _
      | ScmChar _
      ...
      (* ----------------------------------------------------------------------
         2) Quoted forms: 'a => (quote a)
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "quote", ScmPair (sexpr, ScmNil)) ->
          ScmConst sexpr
    
      (* ----------------------------------------------------------------------
         3) Quasiquoted forms: expand them first, then parse again
         ---------------------------------------------------------------------- *)
      ...
      (* ----------------------------------------------------------------------
         5) IF forms [added support for if]
            (if <test> <dit>) => (if <test> <dit> <void>)
            (if <test> <dit> <dif>) => normal
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "if", ScmPair (test, ScmPair (dit, rest))) ->
          (match rest with
           | ScmNil ->
               (* (if test dit) => no else => ScmIf(test, dit, ScmConst ScmVoid) *)
      ...
      (* ----------------------------------------------------------------------
         7) BEGIN forms [added support for begin]
            (begin e1 e2 ... ) => ScmSeq [e1, e2, ...]
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "begin", ScmNil) ->
          (* (begin) => no expressions => void *)
          ScmConst ScmVoid
      | ScmPair (ScmSymbol "begin", sexprs) ->
          (match scheme_list_to_ocaml sexprs with
           | ([], ScmNil) ->
               ...
               ScmConst ScmVoid
           | ([single], ScmNil) ->
               (* (begin single) => just that single expression *)
               tag_parse single
           | (exprs, ScmNil) ->
               (* (begin e1 e2 e3 ...) => sequence of them *)
               ScmSeq (List.map tag_parse exprs)
           | _ ->
               raise (X_syntax "Malformed begin expression"))
      ...
      (* ----------------------------------------------------------------------
         9) DEFINE forms [added support for define]
            (define var expr)
            or (define (f x y ...) body...) => sugar => (define f (lambda (x y ...) body...))
         ---------------------------------------------------------------------- *)
      ...
      | ScmPair (ScmSymbol "define",
                 ScmPair (ScmPair (ScmSymbol f, params), body)) ->
          (* (define (f ...) body...) => expand to (define f (lambda (...) body...)) *)
          if (is_reserved_word f)
          then raise (X_syntax "cannot define a reserved word")
          else
            let lambda_form =
              ScmPair (ScmSymbol "lambda", ScmPair (params, body)) in
            let expanded =
              ScmPair (ScmSymbol "define",
                       ScmPair (ScmSymbol f,
                                ScmPair (lambda_form, ScmNil))) in
            tag_parse expanded
      | ScmPair (ScmSymbol "define", _) ->
          raise (X_syntax "Malformed define expression")
    
      (* ----------------------------------------------------------------------
         10) LAMBDA forms (already partly implemented)
             (lambda (p1 p2 ... ) body...) => Simple
             (lambda (p1 p2 ... . opt) body...) => Opt
             (lambda p body...) => (lambda ( ) body...) with p as opt
         ---------------------------------------------------------------------- *)
      ...
      | ScmPair (ScmSymbol "lambda", ScmPair (params, exprs)) ->
          ...
          (match (scheme_list_to_ocaml params) with
           ...
           | _ ->
               raise (X_syntax "invalid parameter list"))
    
      (* ----------------------------------------------------------------------
         11) let [added support for let]
            (let ((v1 e1) (v2 e2) ...) body...)  =>
              ((lambda (v1 v2 ...) body...) e1 e2 ...)
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "let", ScmPair (bindings, body)) ->
          let (binding_list, binding_tail) = scheme_list_to_ocaml bindings in
          if binding_tail <> ScmNil
          then raise (X_syntax "malformed let: bad bindings list");
          let (vars, vals) =
            List.fold_right
              (fun binding (vars_acc, vals_acc) ->
                 match binding with
                 | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) ->
                     if is_reserved_word var
                     then raise (X_syntax "cannot bind a reserved word in let")
                     else (var :: vars_acc, expr :: vals_acc)
                 | _ ->
                     raise (X_syntax "malformed let binding"))
              binding_list
              ([], []) in
          let lambda_form =
            ScmPair (ScmSymbol "lambda",
                     ScmPair (scheme_sexpr_list_of_sexpr_list
                                 (List.map (fun v -> ScmSymbol v) vars),
                              body)) in
          let applic_form =
            ScmPair (lambda_form,
                     scheme_sexpr_list_of_sexpr_list vals) in
          tag_parse applic_form
    
      (* ----------------------------------------------------------------------
         12) let* [added support for let*]
            (let* ((v1 e1) (v2 e2) ...) body) =>
              (let ((v1 e1))
                (let* ((v2 e2) ...)
                  body))
            If no bindings => ( (lambda () body) )
         ---------------------------------------------------------------------- *)
         | ScmPair (ScmSymbol "let*", ScmPair (bindings, body)) ->
          let (binding_list, binding_tail) = scheme_list_to_ocaml bindings in
          if binding_tail <> ScmNil then
            raise (X_syntax "malformed let*: bad bindings list");
          (match binding_list with
           | [] ->
               (* (let* () body...) => ( (lambda () body...) ) *)
               let lambda_form =
                 ScmPair (ScmSymbol "lambda",
                          ScmPair (ScmNil, body)) in
               tag_parse (ScmPair (lambda_form, ScmNil))
      
           | [single_binding] ->
               (* (let* ((var expr)) body...) => simply (let ((var expr)) body...) *)
               let expanded =
                 ScmPair (ScmSymbol "let",
                          ScmPair (ScmPair (single_binding, ScmNil),
                                   body)) in
               tag_parse expanded
           ...
           | binding1 :: rest ->
               ...
               let expanded =
                 ScmPair (ScmSymbol "let",
                   ScmPair (ScmPair (binding1, ScmNil),
                     ScmPair (
                       ScmPair (ScmSymbol "let*",
                                ScmPair (scheme_sexpr_list_of_sexpr_list rest,
                                         body)),
                       ScmNil))) in
               tag_parse expanded
          )
      ...
      (* ----------------------------------------------------------------------
         13) letrec [added support for letrec]
      ...
      | ScmPair (ScmSymbol "letrec", ScmPair (bindings, body)) ->
          let (binding_list, binding_tail) = scheme_list_to_ocaml bindings in
          if binding_tail <> ScmNil then
            raise (X_syntax "malformed letrec: bad bindings list");
          (* separate out vars and vals *)
          let (vars, vals) =
            List.fold_right
              (fun binding (vars_acc, vals_acc) ->
                 match binding with
                 | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) ->
                     if is_reserved_word var
                     then raise (X_syntax "cannot bind a reserved word in letrec")
                     else (var :: vars_acc, expr :: vals_acc)
                 | _ ->
                     raise (X_syntax "malformed letrec binding"))
              binding_list
              ([], []) in
      
          (* 1) create `'whatever` as a sexpr placeholder *)
          let placeholder =
            ScmPair (ScmSymbol "quote",
                     ScmPair (ScmSymbol "whatever", ScmNil))
          in
          
          (* 2) build the let-bindings, e.g. (v1 'whatever) (v2 'whatever) ... *)
          let placeholder_bindings =
            scheme_sexpr_list_of_sexpr_list
              (List.map (fun v ->
                 ScmPair (ScmSymbol v,
                          ScmPair (placeholder, ScmNil)))
               vars)
          in
      
          (* 3) build (set! v e) forms *)
          let set_forms =
            List.map2
              (fun v e ->
                 ScmPair (ScmSymbol "set!",
          ...
          (* 4) body => (begin (set! ...) ... user-body...) *)
          let user_body_forms = fst (scheme_list_to_ocaml body) in
          let all_body_forms = set_forms @ user_body_forms in
          let body_seq =
            match all_body_forms with
            | [] -> ScmVoid
            | [single] -> single
            | more ->
                ScmPair (ScmSymbol "begin",
                         scheme_sexpr_list_of_sexpr_list more)
          in
      
          (* 5) build the final (let ((v1 'whatever) ...) (begin set! ...)) expansion *)
          let expanded =
            ScmPair (ScmSymbol "let",
                     ScmPair (placeholder_bindings,
                              ScmPair (body_seq, ScmNil)))
          in
          tag_parse expanded
      
      (* ----------------------------------------------------------------------
         14) AND forms (already in code w/ macro_expand_and_clauses)
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "and", ScmNil) ->
          tag_parse (ScmBoolean true)
      | ScmPair (ScmSymbol "and", exprs) ->
          (match (scheme_list_to_ocaml exprs) with
           | (expr :: exprs, ScmNil) ->
              tag_parse (macro_expand_and_clauses expr exprs)
           | _ ->
              raise (X_syntax "malformed and-expression"))
    
      (* ----------------------------------------------------------------------
         15) COND forms (already calls macro_expand_cond_ribs)
         ---------------------------------------------------------------------- *)
      | ScmPair (ScmSymbol "cond", ribs) ->
          tag_parse (macro_expand_cond_ribs ribs)
    
      (* ----------------------------------------------------------------------
         16) Application: (proc arg1 arg2 ...)
            If proc is a symbol in reserved_word_list => error, else parse
         ---------------------------------------------------------------------- *)
      | ScmPair (proc, args) ->
          let proc =
            (match proc with
          ...
          (match scheme_list_to_ocaml args with
           ...
           | _ ->
               raise (X_syntax "malformed application"))
    
      (* ----------------------------------------------------------------------
         17) Fallback: unknown form
         ---------------------------------------------------------------------- *)
      | sexpr ->
          raise (X_syntax
                   (Printf.sprintf
...
(* run this first *)
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      (* var-def: note we currently treat all definitions as global => Free *)
      | ScmVarDef (Var v, expr) ->
        ScmVarDef' (Var'(v, Free), run expr params env)

      (* lambda simple *)
      | ScmLambda (lambda_params, Simple, body) ->
        let body' = run body lambda_params (params :: env) in
        ScmLambda' (lambda_params, Simple, body')

      (* lambda opt *)
      | ScmLambda (lambda_params, Opt opt, body) ->
        let all_params = lambda_params @ [opt] in
        let body' = run body all_params (params :: env) in
        ScmLambda' (lambda_params, Opt opt, body')
  ...
  let annotate_tail_calls =
    let rec run in_tail expr' =
      ...
      | ScmVarSet'(var', e) ->
        ScmVarSet'(var', run false e)
  
      | ScmVarDef'(var', e) ->
        ScmVarDef'(var', run false e)
  
      | ScmBoxSet'(var', e) ->
        ScmBoxSet'(var', run false e)
      ...
      | ScmSeq' exps ->
        (match exps with
         | [] -> ScmSeq' []
         | [single] -> ScmSeq' [run in_tail single]
         | _ ->
           (* all but last are non-tail, last is tail if in_tail = true *)
           let rec process_seq = function
             | [] -> []
             | [last] -> [run in_tail last]
             | first :: rest -> run false first :: process_seq rest
           in
           ScmSeq'(process_seq exps)
        )
  
      | ScmOr' exps ->
        (match exps with
         | [] -> ScmOr' []
         | [single] -> ScmOr' [run in_tail single]
         | _ ->
           (* same idea: all but last -> non-tail, last -> depends on in_tail *)
           let rec process_or = function
             | [] -> []
             | [last] -> [run in_tail last]
             | first :: rest -> run false first :: process_or rest
           in
           ScmOr'(process_or exps)
        )
      ...
      | ScmApplic'(proc, args, _) ->
        ...
        let args' = List.map (fun a -> run false a) args in
  ...
  let make_sets =
    ...
    let rec auto_box expr =
      ...
      | ScmSeq' exprs ->
         ScmSeq' (List.map auto_box exprs)
    
      | ScmVarSet' (v, expr) ->
         ScmVarSet' (v, auto_box expr)
    
      | ScmVarDef' (v, expr) ->
         ScmVarDef' (v, auto_box expr)
      ...
      | ScmLambda' (params, Simple, expr') ->
         ...
         let new_body =
           ...
           | _, _ -> ScmSeq' (new_sets @ [new_body])
         in
         ScmLambda' (params, Simple, new_body)
    
      (* ------------------------------------------------------------------
         2) Lambda Opt
            Similar logic, but we treat (params @ [opt]) as the full param list
         ------------------------------------------------------------------ *)
      | ScmLambda' (params, Opt opt, expr') ->
         let all_params = params @ [opt] in
    
         (* find which vars need boxing *)
         let box_these =
           List.filter
         ...
         (* recursively auto_box the body, then transform gets/sets of box_these *)
         let new_body =
           List.fold_left
             (fun body name -> box_sets_and_gets name body)
             (auto_box expr')
         ...
         let rec make_sets_opt i to_box = function
           | [] -> []
           | param :: rest_params ->
              let rest = make_sets_opt (i + 1) to_box rest_params in
              if List.mem param to_box
              then
                let v = Var' (param, Param i) in
                ScmVarSet' (v, ScmBox' v) :: rest
              else
                rest
         in
    
         let new_sets = make_sets_opt 0 box_these all_params in
...
module Code_Generation (* : CODE_GENERATION *) = struct
  ...
  let const_repr sexpr loc table = match sexpr with
    ...
    | ScmString str -> (*Implemented*)
      let length = String.length str in
      let meta = [RTTI "T_string"; Quad length; ASCII str] in
      (meta, 1 + word_size + length)
    ...
    | ScmNumber (ScmFraction (numerator, denominator)) ->
      let meta = [RTTI "T_fraction"; Quad numerator; Quad denominator] in
      (meta, 1 + 2 * word_size)
    ...
    | ScmVector s ->
      ...
      let sub_ptrs =
        List.map (fun s -> ConstPtr (search_constant_address s table)) s
      in
      let meta = [RTTI "T_vector"; Quad len] @ sub_ptrs in
      (meta, 1 + word_size + (len * word_size))
    ...
    | ScmPair (car, cdr) ->
      ...
      let meta = [RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr] in
      (meta, 1 + 2 * word_size);;
...
(* Function to collect free variables from a list of semantic expressions *)
  let collect_free_vars expressions =
    (* Recursive helper to gather free variables from a single expression *)
    let rec extract_vars expr =
      match expr with
      | ScmVarGet' (Var' (name, Free))
      | ScmBox' (Var' (name, Free))
      | ScmBoxGet' (Var' (name, Free)) ->
          [name]  (* Free variable identified *)
      
      | ScmIf' (test_expr, then_expr, else_expr) ->
          (extract_vars test_expr) @ 
          (extract_vars then_expr) @ 
          (extract_vars else_expr)
      
      | ScmSeq' expr_list
      | ScmOr' expr_list ->
          accumulate_vars expr_list
      
      | ScmVarSet' (Var' (name, Free), value_expr)
      | ScmVarDef' (Var' (name, Free), value_expr)
      | ScmBoxSet' (Var' (name, Free), value_expr) ->
          name :: (extract_vars value_expr)
      
      | ScmVarSet' (_, value_expr)
      | ScmVarDef' (_, value_expr)
      | ScmBoxSet' (_, value_expr) ->
          extract_vars value_expr
      
      | ScmLambda' (_, _, body_expr) ->
          extract_vars body_expr
      
      | ScmApplic' (proc_expr, arg_exprs, _) ->
          (extract_vars proc_expr) @ 
          (accumulate_vars arg_exprs)
      
      | _ ->
          []  (* Other expressions do not contribute to free variables *)
    
    (* Helper function to process a list of expressions *)
    and accumulate_vars exprs =
      List.fold_left (fun acc expr -> acc @ (extract_vars expr)) [] exprs
    in
    
    (* Retrieve global variable names from the global_bindings_table *)
    let global_vars = List.map fst global_bindings_table in
    
    (* Gather free variables from all expressions and include global variables *)
    let gathered_vars = accumulate_vars expressions in
    
    (* Combine and remove duplicates *)
    remove_duplicates (global_vars @ gathered_vars);;
  ...
  (*custom labels*)
  let make_lambda_opt_shift_entry =
    make_make_label ".L_lambda_opt_shift_entry";;
  
  let make_lambda_opt_shift_exit =
    make_make_label ".L_lambda_opt_shift_exit";;
  
  let make_lambda_opt_extra_shift_process =
    make_make_label ".L_lambda_opt_extra_shift_process";;
  
  let make_end_lambda_opt_extra_shift_process =
    make_make_label ".L_lambda_opt_extra_shift_process_end";;
  (*end of custom labels*)
    ...
    let code_gen exprs' =
      ...
      let rec run params env = function
        (* ====================================================================== *)
        (* 1) Constants, variable-get cases                                      *)
        (* ====================================================================== *)
        | ScmConst' sexpr ->
        ...
        | ScmOr' exprs' -> (*implemented*)
           let label_end = make_or_end () in
           (match list_and_last exprs' with
            | Some (exprs_init, last_expr) ->
               let multiple_exprs_code =
                 String.concat ""
                   (List.map
                      (fun e ->
                        let code_e = run params env e in
                        code_e
                        ^ "\tcmp rax, sob_boolean_false\n"
        ...
        | ScmVarSet' (Var' (v, Param minor), ScmBox' _) ->
           "\n\tmov rdi, 8\n\
            \tcall malloc\n\
            \tmov rbx, PARAM("
           ^ string_of_int minor ^ ")\t; param " ^ v ^ "\n\
            \tmov [rax], rbx\t; box " ^ v ^ "\n\
            \tmov PARAM(" ^ string_of_int minor ^ "), rax\t; replace param with box\n\
            \tmov rax, sob_void\n"
        ...
        | ScmBoxGet' var' ->
           (run params env (ScmVarGet' var')) ^ "\tmov rax, qword [rax]\n"
        ...
        | ScmLambda' (params', Simple, body) ->
           ...
           Printf.sprintf
             "\tmov rdi, (1 + 8 + 8)\t; sob closure\n\
              ...
              \tmov SOB_CLOSURE_CODE(rax), %s\n\
              \tjmp %s\n\
              %s:\t; lambda-simple body\n\
              \tcmp qword [rsp + 8 * 2], %d\n\
              \tje %s\n\
              \tpush qword [rsp + 8 * 2]\n\
              \tpush %d\n\
              ...
              %s\
              \tleave\n\
              \tret AND_KILL_FRAME(%d)\n\
              %s:\t; new closure is in rax\n"
             params
             (env + 1)
             label_loop_env
             env
        ...
        | ScmLambda' (params', Opt opt, body) ->
           ...
           and label_exact_shifting_loop   = make_lambda_opt_shift_entry ()
           and label_exact_shifting_loop_end = make_lambda_opt_shift_exit ()
           and label_more_shifting_loop    = make_lambda_opt_extra_shift_process ()
           and label_more_shifting_loop_end= make_end_lambda_opt_extra_shift_process ()
           ...
           ^ Printf.sprintf
               "\tcmp qword [rsp + 8 * 2], %d ;\t check num of args\n\
                \tje %s ;\t if equal, go to exact arity\n\
                \tjg %s ;\t if greater than, go to arity more\n\
                \tpush qword [rsp + 8 * 2] ;\t else, throw opt arity error\n\
                \tpush %d\n\
                \tjmp L_error_incorrect_arity_opt\n\
                %s:\n"
               (List.length params') label_arity_exact label_arity_more
        ...
        | ScmApplic' (proc, args, Non_Tail_Call) ->
           let args_code =
             String.concat ""
               (List.map
                  (fun arg ->
                    let code_arg = run params env arg in
                    code_arg ^ "\tpush rax\n")
                  (List.rev args))
      ...
      (* ======================================================================== *)
      (* 6) The 'runs' function & final assembly integration                      *)
      (* ======================================================================== *)
      and runs params env exprs_list =
        List.map
          (fun single_expr ->
            let partial_code = run params env single_expr in
            partial_code
            ^ "\n\tmov rdi, rax\n\
      ...
      (* Accumulate all code segments into one big string. *)
      let codes = runs 0 0 exprs' in
      let code_body = String.concat "\n" codes in
    
      (* Prepend prologue, constants, free-vars table, etc., then append epilogue. *)
      let code =
